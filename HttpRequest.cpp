...
int HttpRequest::HttpRequestExec(const char* strMethod, const char* strUrl, const char* strData, char* strResponse)
{
    ...
    if (m_iSocketFd < 0 ) { 
		DebugOut("error! Error code: %d Error message: %s\n", errno, strerror(errno));
		return 0;
	}
	...
	if(iRet > 0) {
		...
		os_close(m_iSocketFd);
		m_iSocketFd = INVALID_SOCKET; 
		DebugOut("error! iRet:%d Error code: %d Error message: %s\n", iRet, errno, strerror(errno));
		return 0;
	}
...
char* HttpRequest::HttpDataTransmit(char *strHttpHead, const int iSockFd)
{
	...
	int ret = send(iSockFd,strHttpHead,strlen(strHttpHead),0); 
	free(strHttpHead);
	strHttpHead = NULL;
	if (ret < 0) { 
